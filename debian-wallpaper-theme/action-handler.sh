...
case "$1" in
    ...
    install)
        ...
        if [ -f ${NEW_BACKGROUND_PATH} ]; then
                sudo -u torizon DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(id -u torizon)/bus" gsettings set org.gnome.desktop.background picture-uri 'file://'${NEW_BACKGROUND_PATH}
